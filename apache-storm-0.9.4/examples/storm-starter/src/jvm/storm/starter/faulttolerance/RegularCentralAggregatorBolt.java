...
public class RegularCentralAggregatorBolt extends BaseRichBolt {
	...
	public RegularCentralAggregatorBolt(String stream) {
			...
			if (now - delayVsLastPushTime_.get(delay) >= delay) {
				...
				for (String character : counts.keySet()) {
					...
					if (enableStormsTimeoutMechanism_) {
						collector_.emit(outputStream_, anchors, new Values(character, counts.get(character)));
					} else {
						collector_.emit(outputStream_, new Values(character, counts.get(character)));
					}
					updatePushTime = true;
				}
