...
public class SimpleCentralAggregatorBolt extends BaseRichBolt {
	...
	// Counts of characters and its Tuples which are used to later
	...
	public SimpleCentralAggregatorBolt(String stream) {
		...
		if (now - lastPushTime_ >= WINDOW_LENGTH) {
			for (String character : counts_.keySet()) {
				if (enableStormsTimeoutMechanism_) {
					...
					collector_.emit(outputStream_, anchors, new Values(character, counts_.get(character)));
				} else {
					collector_.emit(outputStream_, new Values(character, counts_.get(character)));
				}
